const questions = [
    ...
    { letter: "k", answer: "koala", status: 0, question: "CON LA K. Animal marsupial originario de Australia"},
    { letter: "k", answer: "kiwi", status: 0, question: "CON LA K. Fruta oval, con piel delgada de color verde parduzco y densamente cubierta de unos pelillos rígidos y cortos de color marrón"}],
    ...
    { letter: "w", answer: "whisky", status: 0, question: "CON LA W. Bebida alcohólica originaria de Escocia"},
    { letter: "w", answer: "washington", status: 0, question: "CON LA W. Capital de los Estados Unidos"}],
...
function pasapalabra() {
    ...
    wrongAnswers = 0
    endGame = false
    while (endGame==false && totalAnswers<27) {
        
    
        for(i=0; i<chosenQuestions.length; i++) {
            if(chosenQuestions[i].status==0) {
                let newQuestion = prompt(chosenQuestions[i].question)
                if(newQuestion==null) {
                    alert(`No es correcto. La respuesta correcta era "${chosenQuestions[i].answer}"`)
                    chosenQuestions[i].status=1
                    wrongAnswers += 1
                    totalAnswers += 1
                }
                else if(newQuestion.toLowerCase()==chosenQuestions[i].answer) {
                    alert('¡Correcto! Ganas un punto.')
                    chosenQuestions[i].status=1
                    correctAnswers += 1
                    totalAnswers += 1
                }
                else if(newQuestion.toLowerCase()=='pasapalabra' || newQuestion == "") {
                    alert('Pasamos')
                }
                else if(newQuestion.toLowerCase()=='end') {
                    alert('Gracias por jugar')
                    endGame = true
                    break;
                }
                else if(newQuestion!=chosenQuestions[i].answer) {
                    alert(`No es correcto. La respuesta correcta era "${chosenQuestions[i].answer}"`)
                    chosenQuestions[i].status=1
                    wrongAnswers += 1
                    totalAnswers += 1
...
function startGame() {
    ...
    pasapalabra();
    if(endGame == false) {  
        alert(`Has contestado correctamente ${correctAnswers} palabras y has fallado ${wrongAnswers} palabras.`)     
...
function sortRanking() {
    ...
    console.log('Los jugadores que han contestado más preguntas correctamente son:')
    for (i=0; i<ranking.length; i++) {
        console.log(`${ranking[i].person}: ${ranking[i].answers}`)
    }
}
